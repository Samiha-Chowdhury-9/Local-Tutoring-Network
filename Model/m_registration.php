...
function registerTutor($username, $pass, $email, $edu, $inst, $exp, $sub, $bio) {
    ...
    $sql1 = "INSERT INTO users (username, password, email, role) VALUES ('$username', '$pass', '$email', 'tutor')";
    if(mysqli_query($conn, $sql1)){
        $uid = mysqli_insert_id($conn);
        
        $sql2 = "INSERT INTO tutor_profiles (user_id, education_background, institution, experience, subjects, short_bio) VALUES ('$uid', '$edu', '$inst', '$exp', '$sub', '$bio')";
        if(mysqli_query($conn, $sql2)) return true;
    }
    return "Error: " . mysqli_error($conn);
}
